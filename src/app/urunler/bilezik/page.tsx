...
export default function Page() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, 'bilezik'))
  ...
  return (
    <div className="min-h-screen bg-brand-black">
      <div className="container mx-auto px-3 sm:px-4 lg:px-8 pt-8 pt-12 lg:pt-30 pb-8 xs:pb-12">
        <h1 className="text-xl sm:text-xl lg:text-3xl xl:text-4xl font-bold text-brand-gold text-center mb-6 sm:mb-8">BİLEZİK</h1>
        ...
        {sortedProducts.length === 0 ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
            {sortedProducts.map((product) => (
              <Link 
                key={product.id}
                href={`/urunler/bilezik/${product.id}`}
